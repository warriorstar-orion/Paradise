...
/datum/admin_ai_template/proc/apply(mob/living/target, client/user)
	if (QDELETED(target) || !isliving(target))
		to_chat(user, "<span class='notice'>Invalid target for AI controller.</span>")
		return
...
/datum/admin_ai_template/proc/apply_controller(mob/living/target, client/user)
	if (QDELETED(target))
		to_chat(user, "<span class='warning'>Target stopped existing while you were answering prompts.</span>")
		return
...
/datum/admin_ai_template/hostile_ranged/proc/setup_ranged_attacks(mob/living/target, client/user)
	...
	var/static/list/all_projectiles = subtypesof(/obj/item/projectile)
