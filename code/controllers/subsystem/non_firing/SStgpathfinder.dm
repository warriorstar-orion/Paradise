...
/datum/controller/subsystem/tgpathfinder/Initialize()
	space_type_cache = typecacheof(/turf/space)
	// para-port
	// return SS_INIT_SUCCESS

/datum/controller/subsystem/tgpathfinder/stat_entry(msg)
...
/datum/controller/subsystem/tgpathfinder/fire(resumed)
	if(!resumed)
		currentrun = active_pathing.Copy()
...
/datum/controller/subsystem/tgpathfinder/proc/swarmed_pathfind(atom/movable/caller, atom/end, max_distance = 30, mintargetdist = 0, age = MAP_REUSE_INSTANT, access = list(), simulated_only = TRUE, turf/exclude, skip_first = TRUE, list/datum/callback/on_finish)
...
/datum/controller/subsystem/tgpathfinder/proc/path_map_cache(turf/target, datum/path_map/hand_back)
	// Cache our path_map
	if(!target || !hand_back)
		return
	source_to_maps[target] += list(hand_back)

/datum/controller/subsystem/tgpathfinder/proc/path_map_fill(turf/target, datum/path_map/fill_into, datum/path_map/hand_back)
...
/datum/controller/subsystem/tgpathfinder/proc/build_map(atom/movable/caller, turf/source, max_distance = 30, access = list(), simulated_only = TRUE, turf/exclude, list/datum/callback/on_finish)
...
/datum/controller/subsystem/tgpathfinder/proc/can_pass_build_map(datum/can_pass_info/pass_info, turf/source, max_distance = 30, simulated_only = TRUE, turf/exclude, list/datum/callback/on_finish)
	var/datum/pathfind/sssp/path = new()
...
/datum/controller/subsystem/tgpathfinder/proc/run_pathfind(datum/pathfind/run)
	active_pathing += run
	return TRUE
...
/datum/controller/subsystem/tgpathfinder/proc/get_valid_map(datum/can_pass_info/pass_info, turf/target, simulated_only = TRUE, turf/exclude, age = MAP_REUSE_INSTANT, min_range = -INFINITY, include_building = FALSE)
...
/datum/controller/subsystem/tgpathfinder/proc/get_valid_maps(datum/can_pass_info/pass_info, turf/target, simulated_only = TRUE, turf/exclude, age = MAP_REUSE_INSTANT, min_range = -INFINITY, include_building = FALSE)
	// Walk all the maps that match our caller's turf OR our target's
	// Then hold onto em. If their cache time is short we can reuse/expand them, if not we'll have to make a new one
