...
/proc/get_hearers_in_view_old(view_radius, atom/source)
...
/proc/get_hearers_in_radio_ranges_old(list/obj/item/radio/radios)
	...
	for(var/obj/item/radio/radio as anything in radios)
		. |= get_hearers_in_view_old(radio.canhear_range, radio)
...
/atom/proc/benchmark_new_old(seconds = 5)
	...
	message_admins("get_hearers_in_view_old(10, src) got [iterations] iterations in [seconds] seconds!")

	BENCHMARK_START

	while(BENCHMARK_LOOP)
		get_hearers_in_view_old(3, src)
		iterations++

	message_admins("get_hearers_in_view_old(3, src) got [iterations] iterations in [seconds] seconds!")

	var/list/all_radios = list()
	for(var/channel in SSradio.frequencies)
		var/datum/radio_frequency/frequency = SSradio.frequencies[channel]
		for(var/obj/item/radio/radio in frequency.devices["[RADIO_CHAT]"])
			all_radios += radio
	BENCHMARK_START
