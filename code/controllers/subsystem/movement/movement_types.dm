...
/datum/move_loop/has_target/jps/proc/recalculate_path()
	...
	if(SStgpathfinder.pathfind(moving, target, max_path_length, minimum_distance, access, simulated_only, avoid, skip_first, diagonal_handling, on_finish = on_finish_callbacks))
		is_pathing = TRUE
...
// para-port: Diposal holder move loop elided for now
