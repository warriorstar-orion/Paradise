...
/datum/controller/subsystem/ticker/proc/setup_news_feeds()
	...
	for(var/loc_type in subtypesof(/datum/trade_destination))
		...
		GLOB.trade_destinations[loc_type] = D
		GLOB.weighted_randomevent_locations[loc_type] = length(D.viable_random_events)
		GLOB.weighted_mundaneevent_locations[loc_type] = length(D.viable_mundane_events)
