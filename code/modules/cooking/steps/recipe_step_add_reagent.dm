...
/datum/cooking/recipe_step/add_reagent/check_conditions_met(var/obj/used_item, var/datum/cooking/recipe_tracker/tracker)
	...
	// Potentially might break shit, but we'll see;
	// If we're currently being processed on a cooking machine,
	// look for that case specifically, and just check to make
	// sure we have the amount of reagents we need already, in
	// order to skip this step.
	//
	// This should hopefully make it easier to deal with adding
	// reagents across steps if, for example, you are adding a
	// mix of reagents from a container, or if you added the
	// reagents in two consecutive steps in the wron order (e.g.
	// adding pepper before salt versus salt before pepper,
