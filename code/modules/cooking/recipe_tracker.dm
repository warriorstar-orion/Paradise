...
/datum/cooking/recipe_tracker/proc/process_item(mob/user, obj/used)
	...
	for(var/step_type in valid_steps)
		...
		//
		// this does mean that the feedback message you get is determined by
		// recipe declaration order. right now this is fine because the only
		// distinct step types we have that can operate on the same items are
		// adding specific objects to a container, but if e.g. there are two
		// valid recipes, with one adding an item to the container and the other
		// one just requiring you use the item on the container, you may end up
		// adding the item but getting the message for using it.
		//
		// sojurn tried to reconcile this problem by adding a user prompt at
		// this point, asking which step the player wanted to perform. i want to
		// avoid throwing up interfaces during cooking, especially when
		// unexpected, so for now, we do nothing, and just watch out for
		// situations where two different recipe steps with incompatible end
		// states are valid with the same object.
