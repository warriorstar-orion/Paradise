...
/datum/cooking/recipe_tracker
	...
	/// the UID of relevant items used for determining quality at recipe completion.
...
/datum/cooking/recipe_tracker/proc/process_item(mob/user, obj/used)
	...
	if(sample_step.is_complete(used, src))
		...
		// Empty out the stove data here so that it can
		// be reused from zero for other cooking steps,
		// as well as to prevent cheatiness where a recipe
		// gets all of its cooking time done before it was
		// supposed to in the recipe order
		if(container)
