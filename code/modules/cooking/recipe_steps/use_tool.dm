...
/datum/cooking/recipe_step/use_tool/follow_step(var/obj/added_item, var/obj/item/reagent_containers/cooking/container)
	var/obj/item/tool/our_tool = added_item
	if(our_tool.worksound && our_tool.worksound != NO_WORKSOUND)
		playsound(usr.loc, our_tool.worksound, 50, 1)
	to_chat(usr, "<span class='notice'>You use the [added_item] according to the recipe.</span>")

	if(our_tool.get_tool_quality(tool_type) < tool_quality)
		return to_chat(usr, "<span class='notice'>The low quality of the tool hurts the quality of the dish.</span>")
...
/datum/cooking/recipe_step/use_tool/calculate_quality(var/obj/added_item)
	...
	if(usr.a_intent == I_HELP)
		...
		to_chat(usr, "<span class='notice'>You apply the [added_item] with ill intent, resulting in a worse dish.</span>")
