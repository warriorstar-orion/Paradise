...
/obj/item/reagent_containers/cooking/proc/process_item(obj/I, mob/user, send_message = TRUE)
	...
	switch(tracker.process_item_wrap(I, user))
		...
		if(PCWJ_COMPLETE)
			if(send_message)
				to_chat(user, "<span class='notice'>You finish cooking with the [src].</span>")
			qdel(tracker)
			tracker = null
			clear_cooking_data()
			update_icon()
		...
		if(PCWJ_SUCCESS)
			if(send_message)
				to_chat(user, "<span class='notice'>You have successfully completed a recipe step.</span>")
			clear_cooking_data()
			return_value = 1
			update_icon()
		if(PCWJ_PARTIAL_SUCCESS)
			if(send_message)
				to_chat(user, "<span class='notice'>More must be done to complete this step of the recipe.</span>")
		if(PCWJ_LOCKOUT)
			if(send_message)
				to_chat(user, "<span class='notice'>You can't make the same decision twice!</span>")
...
/obj/item/reagent_containers/cooking/proc/do_empty(mob/user, atom/target, reagent_clear = TRUE)
	...
	if(length(contents))
		// TODO: If we want to handle food quality reduction by emptying things out
		// instead of putting them on the right platterware (e.g. spaghetti), handle that
		// here. Removing implicit plates is for v2, though, so that's not happening yet.
...
/obj/item/reagent_containers/cooking/update_icon()
	...
	if(lip)
		add_overlay(image(icon, icon_state = lip, layer = LOW_ITEM_LAYER))
