...
/mob/living/simple_animal/demon/pulse_demon/proc/drain_SMES(obj/machinery/power/battery/smes/S, multiplier = 1)
	//CELLRATE is the conversion ratio between a watt tick and powercell energy storage units.
	...
	if(current_cable)
		...
		if(isapc(current_power) && loc == current_power && do_drain)
			if(drain_APC(current_power) > power_per_regen)
				got_power = TRUE
		else if(istype(current_power, /obj/machinery/power/battery/smes) && do_drain)
